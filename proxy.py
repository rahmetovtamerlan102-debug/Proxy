...
SUPPORT_USERNAME = "ytvqi"  # без @ для контакта
...
def support(c):
    # Отправляем контактную карточку поддержки
    contact = telebot.types.Contact(
        phone_number="",
        first_name="Поддержка",
        last_name="VPN Бота",
        user_id=SUPPORT_USERNAME
    )
    bot.send_contact(
        c.message.chat.id,
        phone_number="",
    ...
    bot.send_message(
        c.message.chat.id,
        f"👤 *Нажми на контакт выше*, чтобы открыть профиль поддержки и написать сообщение.\n\nИли напиши сюда: @{SUPPORT_USERNAME}",
        parse_mode="Markdown"
    )
