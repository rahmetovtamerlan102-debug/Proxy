...
def check_single_proxy(proxy_url):
    try:
        ...
        for _ in range(2):
            proc = subprocess.run(["ping", "-c", "1", "-W", "2", server], capture_output=True, timeout=5)
...
def refresh_proxies():
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        future_to_proxy = {executor.submit(check_single_proxy, proxy): proxy for proxy in raw_proxies}
        for future in as_completed(future_to_proxy):
            proxy = future_to_proxy[future]
            if future.result():
                alive_proxies.append(proxy)
                if len(alive_proxies) % 20 == 0:
                    print(f"✅ Найдено живых: {len(alive_proxies)}")
    if alive_proxies:
        working_proxies = alive_proxies
        print(f"🎉 УСПЕХ! Загружено {len(working_proxies)} живых прокси")
    else:
        print("⚠️ Не найдено живых прокси, сохраняю старый список")
...
def get_live_proxy():
    if not working_proxies:
        return None
    shuffled = working_proxies.copy()
    random.shuffle(shuffled)
    for proxy in shuffled[:20]:
...
def self_ping():
    ...
    url = f"https://{host}/" if host != 'localhost' else f"http://localhost:{PORT}/"
    while True:
        try:
